const actions = {
      ...
      if (docRef.exists) {
        ...
        Goals.forEach((Goal) => {
          ...
          const newGoal = {
            ...
            CreatedAt: Goal.CreatedAt || tmpCreatedAt,
            Source: Goal.Source || '',
            SourceType: Goal.SourceType || '',
          }
  ...
  */
  ...
  /*
      Add Roadblocks to the Meeting.PreviousRoadblocks array
  */
  async AddPreviousRoadblocks({ context }, { GroupID, MeetingID, Roadblocks }) {
    if (!GroupID || !MeetingID || !Roadblocks) { return }
    // set the CreatedAt
    const tmpDate = new Date()
    const tmpCreatedAt = `${tmpDate.getFullYear()}-${tmpDate.getMonth() + 1
      }-${tmpDate.getDate()} ${tmpDate.getHours()}:${(
    ...
    meetingRef.get().then((docRef) => {
      if (docRef.exists) {
        const Meeting = docRef.data()
        // We have 2 potential arrays: existing and new roadblocks
        let newRoadblocks = []
        let existingRoadblocks = Meeting.PreviousRoadblocks ? Meeting.PreviousRoadblocks : []

        // process each roadblock and push them into newRoadblocks
        Roadblocks.forEach((Roadblock) => {
          // Build the Roadblocks @TODO: create a generic function for this to ensure map standardization
          const newRoadblock = {
            Member: Roadblock.Member, //@TODO: this is where it would be nice to call that function I mentioned above
            Title: Roadblock.Title,
            Status: Roadblock.Status || 'new',
            CreatedAt: Roadblock.CreatedAt || tmpCreatedAt,
            Source: Roadblock.Source || '',
            SourceType: Roadblock.SourceType || '',
          }

          // push it to the newRoadblocks array
          newRoadblocks.push(newRoadblock)
        })
        // let's merge the 2 together
        // add newRoadblocks to updateRoadblocks
        let updateRoadblocks = newRoadblocks

        // push into the updateRoadblocks the existing ones that aren't in this new batch
        existingRoadblocks.forEach((existingRoadblock) => {
          // find it in the updateRoadblocks
          let tmpRoadblock = updateRoadblocks.find((item) => {
            return (
              item.Member.Email === existingRoadblock.Member.Email &&
              item.Title === existingRoadblock.Title
            )
          })
          if (!tmpRoadblock) {
            // push it to the list
            updateRoadblocks.push(existingRoadblock)
          }
        })
        /*
            at this point we should have a clean set of updateRoadblocks
            with the newRoadblocks and the existingRoadblocks merged without duplicates
        */

        // update FS document (Firestore & Vuex handles the rest with the listeners)
        meetingRef.update({
          PreviousRoadblocks: updateRoadblocks
        })
      } else {
        // docRef.data() will be undefined in this case
        console.log("Invalid Meeting!");
  ...
  /*
      Update the status of a PreviousRoadblock in Meeting.PreviousRoadblocks
  */
  async SetPreviousRoadblockStatus({ context }, { GroupID, MeetingID, Roadblock, Status }) {
    if (!GroupID || !MeetingID || !Roadblock) { return }
    ...
    meetingRef.get().then((docRef) => {
      if (docRef.exists) {
        const Meeting = docRef.data()
        const tmpPreviousRoadblocks = Meeting.PreviousRoadblocks


        // fetch the index of the item to be updated
        let tmpPreviousRoadblockIndex = tmpPreviousRoadblocks.findIndex((item) => {
          return (item.Member.Email === Roadblock.Member.Email && item.Title === Roadblock.Title)
        })

        // if we don't find it, stop processing
        if (tmpPreviousRoadblockIndex === -1) {
          return
        }

        // buid a newRoadblock to avoid the vuex mutation issue
        let newRoadblock = {}
        for (const [key, value] of Object.entries(Roadblock)) {
          newRoadblock[key] = value
        }
        // set the new status
        newRoadblock.Status = Status

        // splice in the new updated Roadblock
        tmpPreviousRoadblocks.splice(tmpPreviousRoadblockIndex, 1, newRoadblock)
        // update the FS document
        meetingRef.update({
          PreviousRoadblocks: tmpPreviousRoadblocks
        })
        // Vuex/Firestore takes care of the rest
      } else {
        // docRef.data() will be undefined in this case
    ...
    meetingRef.get().then((docRef) => {
      if (docRef.exists) {
        ...
        const newSidebar = {
          ...
          Status: Sidebar.Status || 'new',
          CreatedAt: Sidebar.CreatedAt || tmpCreatedAt,
